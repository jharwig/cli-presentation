// Adapted from https://github.com/yaronn/blessed-contrib/blob/master/examples/dashboard.js
...
import { Grid, createBlessedComponent } from "react-blessed-contrib";

const Donut = createBlessedComponent(contrib.donut);
const Sparkline = createBlessedComponent(contrib.sparkline);
const Bar = createBlessedComponent(contrib.bar);
const Table = createBlessedComponent(contrib.table);
const Lcd = createBlessedComponent(contrib.lcd);
const Line = createBlessedComponent(contrib.line);
const Map = createBlessedComponent(contrib.map);
const Log = createBlessedComponent(contrib.log);
const Gauge = createBlessedComponent(contrib.gauge);
// 'Donut, Sparkline, Bar, Table, Lcd, Line, Map, Log, Gauge'.split(', ').forEach
...
const interval = (list, fn, delay) => {
  list.push(setInterval(fn, delay));
};
...
class App extends Component {
  ...
  componentDidMount() {
    ...
    interval(
      ...
      () => {
        this.setState({
          gauge_percent_two: (this.state.gauge_percent_two + 1) % 100
        });
      },
      200
    );
    ...
    refreshSpark();
    interval(this.timers, refreshSpark, 1000);
    ...
    interval(
      ...
      () => {
        this.setState({
          transactionsLineData: getLineData([
            transactionsData,
            transactionsData1
          ])
        });
    ...
    // interval(this.timers, () => {
    //   this.setState({ lcdValue: Math.round(Math.random() * 100) });
    //   lcdLineOne.setDisplay(
    //     this.state.lcdValue + lcdText[this.state.lcdValue % 12]
    //   );
      ...
      <Grid rows={12} cols={12}>
        {/* <Donut
          ...
          {...{
            ...
            arcWidth: 2,
            yPadding: 0,
            data: updateDonut(this.state.donutPct)
          }}
        /> */}
        {/* <Gauge
          row={6}
          ...
          rowSpan={4}
          colSpan={4}
          {...{ label: "Storage", percent: [80, 20] }}
          data={[this.state.gauge_percent, 100 - this.state.gauge_percent]}
        /> */}
        {/* <Gauge
          row={0}
          col={6}
          rowSpan={4}
          colSpan={6}
          {...{ label: "Deployment Progress", percent: 80 }}
          data={this.state.gauge_percent_two}
        /> */}
        <Sparkline
          ref="sparkline"
          row={0}
        ...
        {/* <Lcd
          ...
          {...{
            ...
            strokeWidth: 0.1,
            elements: 5,
            color: lcdColors[this.state.lcdValue],
            display: this.state.lcdValue + lcdText[this.state.lcdValue % 12],
            elementSpacing: 4,
            elementPadding: 4
          }}
        /> */}
        {/* <Line
          ...
          data={this.state.errorsLineData}
        /> */}
        <Line
          row={0}
          col={0}
          ...
          data={this.state.transactionsLineData}
        />
        {/* <Map
          ref="map"
          row={6}
        ...
        <Log
          ...
          rowSpan={6}
          colSpan={5}
          {...{ fg: "green", selectedFg: "green", label: "Server Log" }}
        />
      </Grid>
    );
  }
